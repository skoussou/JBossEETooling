...
//			String threadPoolExecName = ms.findWebExecutorHelper(new HashMap(){{put(CONNECTOR, "http");}}, cliConnection.getClient());
			
			// type=memory
			ModelNode cliCommand = ms.monitorCommand(new HashMap(){{put(PLATFORM_MBEAN_TYPE, "memory");}});
			...
			// type=memory-pool
			cliCommand = ms.monitorCommand(new HashMap(){{put(PLATFORM_MBEAN_TYPE, "memory-pool");}});
