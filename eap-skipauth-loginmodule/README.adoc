...
A. For now removed completely the need for "kie-server" role when accessing the REST API but on a later stage may return it for access to "ADMIN" KIE SERVER Tasks
...
B. *** TODO *** RESTRICT the requirement for "kie-server" to KIE-SERVER management tasks
...
** Retrieve my tasks
*** Should Fail: curl -X GET "http://localhost:8080/kie-server/services/rest/server/queries/tasks/instances/owners?user=testUser"
*** Should Succeed: curl -u 'ficticiousUser:ficticiousUser' -X GET "http://localhost:8080/kie-server/services/rest/server/queries/tasks/instances/owners"
